"""Novadata Products + FBA Availability Export
================================================

Lädt das tägliche Produkt- und Bestandsfeed von Novadata herunter und
speichert es als gzipped CSV im selben Ordner wie die Margin-Exports.

Setup:
    pip install -r requirements-export.txt

Ausführung:
    python novadata_products_export.py --once
"""
...
DOWNLOAD_URL = (
    ...
    "d68e89d0-f265-456c-bc86-1b5208dd9445"
)
OUTPUT_DIR = Path("./novadata_exports")
FILENAME_PREFIX = "products_export"
SCHEDULE_TIME = "06:30"           # 30 min nach dem Margin-Run
DELETE_OLD_FILES_AFTER_DAYS = 14  # Repo-Hygiene
...
def cleanup_old_files() -> None:
    ...
    for pattern in (f"{FILENAME_PREFIX}_*.csv", f"{FILENAME_PREFIX}_*.csv.gz"):
...
def run_export() -> None:
    ...
    log.info(f"=== Starte Produkt-/FBA-Export ({date_str}) ===")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    raw_path = OUTPUT_DIR / f"_raw_{FILENAME_PREFIX}_{date_str}.csv"
    final_path = OUTPUT_DIR / f"{FILENAME_PREFIX}_{date_str}.csv.gz"

    if not download_file(DOWNLOAD_URL, raw_path):
        log.error("Export abgebrochen.")
        return

    # Sanity-check: muss wie eine CSV aussehen (nicht HTML-Fehlerseite).
    with open(raw_path, "rb") as f:
        head = f.read(512)
    if b"," not in head:
        log.error(
            "Heruntergeladene Datei sieht nicht wie eine CSV aus "
            "(evtl. abgelaufener Link?)."
        )
